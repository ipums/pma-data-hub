...
knitr::include_graphics("images/chirps1.png")
```

We'll draw a polygon around Burkina Faso like this: 

```{r, echo=FALSE}
knitr::include_graphics("images/chirps2.png")
```

And then we'll select daily precipitation data for **20 years** from January 1, 1997 to January 1, 2017. 

```{r, echo=FALSE}
knitr::include_graphics("images/chirps3.png")
```

You'll receive a compressed folder containing one "tif" image for each day in the 20 year timespan: that's 7305 files containing comprising potentially several gigabytes of raster data. We've previously shown how to use the [raster](https://rspatial.org/raster/index.html) package to handle this type of data, but we'll now introduce the [terra](https://rspatial.org/terra/index.html) package as a newer, faster alternative. 
...
```{r}
day1 <- rast("data/chirps_bf/1997-3-1.tif")
day1
```

As you can see, this image contains 132 rows and 176 columns of pixels. The value in each pixel represents the rainfall - in millimeters - for an area 0.05 degrees longitude by 0.05 degrees latitutde (shown in the "resolution" field). If you just want to preview the [values](https://rspatial.github.io/terra/reference/values.html) associated with each pixel, we recommend coercing the default `matrix` as a `data.frame` or `tibble`:
...
The result is a single column with precipitation totals for 23,232 pixels. This is the CHIRPS data we've obtained for *a single day* in our 20-year timespan. Alone, it's not a lot of data:
...
Now, imagine working with data for each of the 7,305 days in our data series. Whereas the total size of the datset from a single day was small - only about 0.2 megabytes - the amount of data required to represent 23,232 pixels from daily collection over a 20-year period could easily overwhelm the amount of memory available to R. 

We'll take two steps to make this load more manageable. To start, we'll simply compile the metadata - not the *values* - for each image in a list. If you haven't done so via the API, this might be a good time to [crop](https://rspatial.github.io/terra/reference/crop.html) each image down to the smallest possible area necessary for your analysis. In our example below, we'll crop each file down to an area between 9.0 and 15.6 degrees latitude and between -6.0 and 2.8 degrees longitude. Meanwhile, we'll also load the metadata for each image into a list called `chirps` with [purrr::map](https://purrr.tidyverse.org/reference/map.html). 
...
chirps <- list.files("data/chirps", full.names = TRUE) %>%
  map(~{
    .x %>%
      rast() %>%
      crop(c(-6 , 2.8, 9 , 15.6), filename=.x, overwrite=TRUE)
  })
```

```{r, include=FALSE}
# Make a raster for each file
chirps <- map(list.files("data/chirps_bf", full.names = T), ~rast(.x))
```
...
The `terra` package contains several [summary methods](https://rspatial.github.io/terra/reference/summarize-generics.html) you can use to quickly explore summary statistics over all of the layers included in our 20-year timespan. For example, `terra::mean` computes the mean value for each pixel across every layer of `chirps`. This can be particularly helpful if you want to create a map showing the 20-year average rainfall for selected enumeration areas relative to neighboring areas:
...
```{r, include = FALSE}
bf_means <- c(
  terra::mean(chirps[[1:1000]]),
  terra::mean(chirps[[1001:2000]]),
  terra::mean(chirps[[2001:3000]]),
  terra::mean(chirps[[3001:4000]]),
  terra::mean(chirps[[4001:5000]]),
  terra::mean(chirps[[5001:6000]]),
  terra::mean(chirps[[6000:7305]])
)

bf_means <- terra::mean(bf_means)
```
...
ggplot() +
  layer_spatial(
    bf_means,
    alpha = if_else(values(bf_means) == 0, 0, 1) # makes 0 values transparent 
  ) +
  layer_spatial(shape, alpha = 0) +
  layer_spatial(gps,  alpha = 0) +
  theme_pma_rainfall(
    title = "Burkina Faso 20-year Average Daily Rainfall",
    subtitle = paste(
      "National precipitation with displaced centroid locations for 2017 PMA",
      "Nutrition Survey enumeration areas"
    )
  ) +
  scale_fill_continuous(low = "#FAEFD1", high = "#00263A") + 
  labs(caption = paste(
    sep = "\n",
    "Climate Hazards Center InfraRed Precipitation with Station data (CHIRPS)",
    "01-01-1997 to 01-01-2017"
  ))
```

For most analytic purposes, though, you'll want to do more than simply *layer* summary data beneath a plot of the PMA enumeration areas. Instead, we'll want to [extract](https://rspatial.github.io/terra/reference/extract.html) the pixels associated with the area covered by the buffer zones we created above. 
...
The result is a `data.frame` containing one row for each pixel that overlaps with our buffer (the top-right and bottom-right pixels are omitted). The column `1997-3-1` gives the rainfall for each pixel, and the column `weight` shows the proportion of each pixel that falls within the buffer. We can think of these `weight` values as probabilities representing the likelihood that each pixel contains the real centroid location for `EA == 7003`. Let's calculate a `weighted.mean` using the probabilities represented by `weight`:

```{r}
day1_7003 %>% 
  summarise(
    EAID = 7003,
    wtd_mean = weighted.mean(`1997-3-1`, weight)
  )
```

This represents the approximate *single-day rainfall total* for March 1, 1997 spatially averaged for all pixels overlapping with the buffer for `EA == 7003`. In order to produce the *20-year average daily rainfall* for this buffer, we'll simply repeat the same process for each day between January 1, 1997 and January 1, 2017 (the entire stack represented by `chirps`). We'll arrange the results in a `tibble`, and we'll replace the default `ID` column with the unique identifier for each `pixel` shown above.

```{r, eval=FALSE}
chirps20_7003 <- extract(
  chirps,
  gps %>% filter(EAID == 7003) %>% vect(),
  weights = TRUE
)

chirps20_7003 <- chirps20_7003 %>%
  tibble() %>%
  dplyr::select(-ID) %>%
...
```{r, echo=FALSE}
chirps20_7003 <- read_rds("data/ea_one.rds") %>% rename(pixel = tile)
chirps20_7003
```

The rainfall total for each day is stored in one of 7,305 columns to the right of `pixel` and `weight` (the dates are arranged in *alphabetical* order, so `1997-1-10` follows `1997-1-1`). We'll use `pivot_longer` to rearrange these data in rows, and then we'll calculate a simple `weighted.mean` to obtain the 20-year average daily rainfall. 

```{r}
chirps20_7003 <- chirps20_7003 %>% pivot_longer(-c(pixel, weight)) 
chirps20_7003   
...
```{r, echo = FALSE}
chirps20_all <- read_rds("data/ea_all.rds.gz")
chirps20_all 
```
...
```{r, layout="l-body-outset", fig.width=10, fig.height=8, preview = TRUE}
gps <- chirps20_all %>% 
  pivot_longer(-c(ID, EAID, weight)) %>% 
  group_by(EAID) %>% 
  summarise(CHIRPS_20 = weighted.mean(value, weight)) %>% 
  full_join(gps, by = "EAID")

ggplot() + 
  layer_spatial(shape, alpha = 0) + 
  layer_spatial(gps, aes(fill = CHIRPS_20, stroke = 0), alpha = 1) + 
  theme_pma_rainfall(
    title = "Burkina Faso 20-year Average Daily Rainfall", 
...
We can also attach these local rainfall totals to the records for individual women and children in the 2017 Burkina Faso nutrition dataset `dat`. At this point, the **simple features collection** class is no longer necessary; we can simply join `CHIRPS_20` directly to `dat`.  

```{r}
gps %>% 
  select(EAID, CHIRPS_20) %>% 
