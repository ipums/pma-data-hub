...
Here, we use `vartype = "ci"` to include a 95% confidence interval set by `level = 0.95`. We also recommend using `proportion = TRUE` and `prop_method = "logit"` to make a Wald-type confidence interval adjustment for discrete variables where the estimated proportion approaches 0% or 100%.^[See [svyciprop](https://rdrr.io/pkg/survey/man/svyciprop.html) for additional methods.]
