...
You'll notice that there are 515 rows in our `conflict` dataset - each representing a specific event with a unique `data_id` number. The variables `event_type` and `sub_event_type` describe different types of recorded events. We'll use the [kableExtra](https://github.com/haozhu233/kableExtra) package to create an easy-to-read table showing the frequency of each event type.
